...
export function SpecialtiesListModal({ specialtiesItems, setSpecialtiesItems }: SpecialtiesListModalProps) {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    toast("Especialidades adicionada!")
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="default" className="flex items-center justify-between">
          {specialtiesItems.length === 0 ? 'Adicionar especialidades' : `${specialtiesItems.length} especialidades adicionadas`}
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="flex flex-col gap-4 py-4">
          ...
          {specialtiesItems.length >= 1 ?
            ...
            <span className="text-sm text-center text-zinc-400">Sem especialidades</span>
          }

          <div className="flex gap-4 border-t border-zinc-400 pt-4">
            <Input placeholder="Especialidade.." className={errors.specialty && 'border border-pink-600'} {...register('specialty')} />
            <Button size={'sm'} variant={'default'} onClick={handleSubmit(onSubmit)} disabled={isLoading} className='bg-blue-600 text-zinc-100 justify-between items-center disabled:cursor-not-allowed'>
              <Plus className="text-zinc-100" />
            </Button>
          </div>
